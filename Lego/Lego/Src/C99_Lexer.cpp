...
T_FlexLexer::T_FlexLexer(T_Source *s) : TScanArray(s), lexer(NULL)
{}
...
TFileLexer::TFileLexer(T_SourceFile *s) : TScanArray(s)
{}
...
bool TFileLexer::Open()
{
   return true;
}

T_Lexeme::T_Group TFileLexer::LexMake(T_Lexeme &l)
{
   FILE *file = ((T_SourceFile*)source)->file;
   if (feof(file)) {
      l.group = T_Lexeme::grEof;
      return l.group;
   }
   l.text = NULL;
   FSCANF(file,"%d %d %d %d %d %d %d ", &l.group, &l.type, &l.pos, &l.line, &l.column, &l.len, &l.value);
   T_Lexeme::T_Group group = l.group;
   int type = l.type;
   ...
   if ((group == T_Lexeme::grOper) || (group == T_Lexeme::grSeparator)) {
      ...
      char* buf_text = (char*)malloc(l.len + 1);
      fgets( buf_text, l.len + 1, file );
      l.text = buf_text;
      l.value = names.Add(buf_text, l);
   } else if (group == T_Lexeme::grInteger) {
      char* buf_text = (char*)malloc(l.len + 1);
      fgets( buf_text, l.len + 1, file );
      int number = atoi(buf_text);
      int index = names.Add(buf_text, l);
      l.text = names.Name(index);
      l.type = 0;
      l.value = Integer(number);
   } else if (group == T_Lexeme::grNumber) {
      char* buf_text = (char*)malloc(l.len + 1);
      fgets( buf_text, l.len + 1, file );
      double number = atof(buf_text);
      int index = names.Add(buf_text, l);
      l.text = names.Name(index);
      l.type = 0;
      l.value = Number(number);

...
Assign                   
                         ...
                         ;
QuestionList             
                         :   <Priority,Question>         { CODE(BegQuestion); }      Question                    { NODE(Trio,(0)); }         <Separator,Colon>           { CODE(MidQuestion); }      Question                    { NODE(Trio,(0),1); }       { MAKE(Question); }      
                         |   
                         ;
Question                 
...
ForPosfix                
                         ...
                         ;
ForStmnt                 
                         :   <Reserv,For>                <Separator,LeftBracket>     { CODE(BegFor); }           ForPrefix                   { CODE(PrefFor); }          { NODE(For,(1)); }          ForCondition                { CODE(ConditionFor); }     { NODE(For,(1),1); }        ForPosfix                   { CODE(PostFor); }          { NODE(For,(1),2); }        DefExpr                     { NODE(For,(1),3); }        { MAKE(For); }           
                         ;
DoStmnt                  
                         :   <Reserv,Do>                 { CODE(BegDo); }            DefExpr                     { NODE(None,(1)); }         <Reserv,While>              <Separator,LeftBracket>     { CODE(MidDo); }            Condition                   { NODE(Do,(1,4)); }         <Separator,RightBracket>    <Separator,Semicolon>       { MAKE(Do); }            
                         ;
WhileStmnt               
                         :   <Reserv,While>              { CODE(BegWhile); }         <Separator,LeftBracket>     Condition                   { NODE(None,(1)); }         <Separator,RightBracket>    { CODE(MidWhile); }         DefExpr                     { MAKE(While); }            { NODE(While,(2,1)); }   
                         ;
GoToStmnt                
